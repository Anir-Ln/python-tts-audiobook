...
class TTS:
	...
	async def chapter_to_audio(self, chapter: Chapter):
		...
		if chapter.title:
			...
			chapter_bytes.write(AudioHelper.generate_pause(CHAPTER_TITLE_PAUSE_DURATION))
...
class AudioBookGenerator:
	...
	async def generate(self):
		...
		for chapter in self.book.chapters[self.start_chapter: self.end_chapter+1]:
			...
			chapter_audio.export(self.out_folder + "/chapters/" + chapter.title + ".mp3")
			book_audio += chapter_audio
			book_audio += AudioSegment.silent(CHAPTER_PAUSE_DURATION)
			chapter.start_time = time
			chapter.end_time = time + chapter_audio.duration_seconds * 1000 + CHAPTER_PAUSE_DURATION
			time = chapter.end_time
			book_ffmetadata += chapter.get_metadata_text() # start_time and end_time should be set
	...
	def _bind_metadata(self, metadata, audiobook_path):
		...
		subprocess.run(cmd)
		# subprocess.run([
    #     'ffmpeg', '-i', audiobook_path, '-i', ffmetadata_path, '-map_metadata', '1', 
    #     '-c', 'copy', audiobook_path
    # ])
...
def main():
	...
	try:
		...
		if start_chapter < 0 or end_chapter > len(toc_titles) - 1 or start_chapter > end_chapter:
